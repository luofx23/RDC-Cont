...
F2u=[[M 0*M 0*M 0*M];[0*M M 0*M 0*M];[0*M 0*M M 0*M];[0*M 0*M 0*M M]]*...
    [[spdiags(f11,0,nx,nx) spdiags(f12,0,nx,nx) spdiags(f13,0,nx,nx) spdiags(f14,0,nx,nx)];
     [spdiags(f21,0,nx,nx) spdiags(f22,0,nx,nx) spdiags(f23,0,nx,nx) spdiags(f24,0,nx,nx)];
...
Gu = K-(F1u+F2u);
sjac =[[-Kx 0*Kx 0*Kx 0*Kx];
       [0*Kx -Kx 0*Kx 0*Kx];
       [0*Kx 0*Kx -Kx 0*Kx];
       [0*Kx 0*Kx 0*Kx -Kx]]*u(1:4*nx);
Gu = [Gu sjac];
Gu = [Gu;[(Kx*uref(1:nx))'/norm(Kx*uref(1:nx)) 0*uref(1:3*nx)' 0]];